...
const searchAPI =
    'https://api.spoonacular.com/recipes/search?apiKey=a16063f4fe2b4f5b952d4a348d15892e';
...
function getCard(params) {
    return ` <div class="md:w-1/2 lg:w-1/3 py-4 px-4"> <a href=/single.html?id =$
        ...
        ready in $ {
            params .readyInMinutes
        }

        minutes
        </div >
        <img src="https://spoonacular.com/recipeImages/${params.id}-556x370.jpg"
...
function appendData(data) {
    // append search value in appropriate place if (data.results.length)
...
function doFetch(query) {
    fetching(true);
    fetch(`${searchAPI}&query=${query}&offset=${resultsArray.length}`).then(function (response) {
